...
- **`PublicWidgetZones.ProductDetailsBottom`** — **correction (found by `integration-auditor` during the post-implementation gate): this design's original claim that no other plugin claims this zone was wrong.** `Nop.Plugin.Misc.Omnisend` (`OmnisendPlugin.cs:36`) already registers it alongside `PublicWidgetZones.BodyStartHtmlTagAfter`, rendering an inline tracking `<script>` (or nothing) via `CustomViewComponent.cs`. GIL-001 does not claim this zone (it uses `productdetails_before_collateral` per spec §3), so the original claim was right about GIL-001 but never checked Omnisend. Runtime impact is still safe — `WidgetViewComponent`'s zone-render loop supports multiple simultaneous widgets per zone by design, and Omnisend's output (a `<script>` tag or empty content) cannot visually collide with this plugin's markup — but the verification itself was incomplete. Corrected here so a future change doesn't trust the original "no collision" claim as exhaustive.
